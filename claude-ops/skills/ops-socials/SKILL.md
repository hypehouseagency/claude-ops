---
name: ops-socials
description: Public social channels command center — X/Twitter, LinkedIn, Threads, Bluesky, Mastodon. Routes reads via x-research-skill + x-mcp, posts via Typefully (ban-safe, multi-platform, social set 294555), LinkedIn voice via linkedin-skills. Stages drafts; never auto-publishes. Use when the user says socials, post, tweet, thread, draft, schedule a post, search X, what is X saying about, AI twitter, AI news, monitor a profile, LinkedIn post, or runs /ops-socials. NOT for private DMs/email/Slack/WhatsApp — those go through /ops-comms.
...
2. **Stage drafts; never auto-publish.** Per Sam's outbound-comms doctrine, even public posts go stage→approve. Return the typefully.com link (`https://typefully.com/?a=294555&d=<draft_id>`) and wait for plain-chat approval (`ok`, `send`, `ship it`, `post it`, `go`, `do it`). The campaign-level pre-approval in CLAUDE.md does NOT cover individual sends.
3. **No cookie-auth scraping, no Puppeteer/Playwright automation against X.** Suspension risk on the marketing account (`@SamFeldtMusic`).
...
### "Anyone @-ing me?"
```
mcp__x-mcp__get_mentions({ user_id: "<@SamFeldtMusic id>" })
```
...
mcp__typefully__typefully_create_draft({
  content: "Hook tweet.\n---\nSecond tweet.\n---\nThird tweet.",
  social_set_id: 294555,
  platforms: ["x"]
})
```
Return `https://typefully.com/?a=294555&d=<draft_id>` and wait for `ok`.
...
2. Push the drafted text to Typefully:
   ```
   mcp__typefully__typefully_create_draft({ content, social_set_id: 294555, platforms: ["linkedin"] })
   ```
...
```
mcp__typefully__typefully_create_draft({ content, social_set_id: 294555, schedule_date: "next-free-slot" })
# or ISO: "2026-05-24T09:00:00Z"
mcp__typefully__typefully_get_queue({ social_set_id: 294555, start_date, end_date })  # inspect
```
...
If x-mcp returns 000/timeout: `launchctl kickstart -k gui/$UID/com.user.mcp-proxy` and wait ~5s.
...
- **X app**: `x-mcp-sam` (ID `32972280`) on `@SamFeldtMusic`, Pay-Per-Use tier
- **Typefully** key + default social set: `~/.config/typefully/config.json` (social set `294555` — Sam Feldt, X + LinkedIn linked)
- **Sub-skills**: `~/.claude/skills/x-research-skill/`, `~/.claude/skills/x-article-publisher-skill/`, `~/.claude/skills/linkedin-skills/`
- **Proxy LaunchAgent**: `com.user.mcp-proxy` (servers.json at `~/.claude/mcp-proxy/servers.json`)
